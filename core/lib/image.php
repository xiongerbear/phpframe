...
//验证码字体文件
define('VERIFY_FONTFILE',FRAME.'/core/lib/ttf/STHUPO.ttf');
//验证码字体大小
...
/**********水印配置项**********/
//水印图路径
define('WATER_IMAGE', FRAME.'/core/lib/ttf/water.png');
//水印位置
define('WATER_POS', 5);
//水印透明度
...
define('WATER_FONTFILE',FRAME.'/core/lib/ttf/STHUPO.TTF');
//水印文字字符编码
...
class image{
	...
	Static Public function verify($length=null,$width=null,$height=null){
		...
		for ($i=0; $i<$length; $i++) { 
			...
			$font = substr($seed,mt_rand(0,$len-1),1);
			$x = $size * $i + $left;
